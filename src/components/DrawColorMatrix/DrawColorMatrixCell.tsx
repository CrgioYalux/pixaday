...
const DrawColorMatrixCell: React.FC<DrawColorMatrixCellProps> = ({ value, position, usingPencil, setUsingPencil }) => {
    ...
    return (
        <div
            ...
            onTouchMove={(event) => {
                const target = document.elementFromPoint(event.changedTouches[0].clientX, event.changedTouches[0].clientY) as HTMLDivElement & {
                    dataset: Partial<{
                        positionX: number,
                        positionY: number,
                    }>
                ...
                if (usingPencil && target.dataset.positionX && target.dataset.positionY) {
